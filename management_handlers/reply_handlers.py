...
async def add_reply(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if len(context.args) >= 2:
        ...
        await update.message.reply_text("请提供关键词和回复。例如: /add_re 退款:会退款的请耐心等待")
...
async def remove_reply(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if context.args:
        ...
        await update.message.reply_text("请提供要删除回复的关键词。例如: /del_re 退款")
...
add_reply_handler = CommandHandler("add_re", add_reply)
remove_reply_handler = CommandHandler("del_re", remove_reply)
list_replies_handler = CommandHandler("list_re", list_replies)